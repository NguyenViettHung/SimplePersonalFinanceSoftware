...
void docFileGiaoDich(){
    ...
    while (fgets (line, sizeof(line), file)){ 
        ...
        char *token_ghichu = strtok(NULL, ", \n");
        if (token_ghichu != NULL) {
            strcpy(gd_temp.ghi_chu, token_ghichu);
        } else {
            strcpy(gd_temp.ghi_chu, ""); // Gán chuỗi rỗng nếu không có ghi chú
        }
    ...
    // Đồng bộ lại sức chứa sau khi nạp file
...
void ghiFileGiaoDich(){
    ...
    if (file == NULL){
        printf ("Loi: Khong the mo file giao_dich.txt de ghi!\n");
        return;
    }

    // Ghi mảng thu
    for (int i = 0; i < so_luong_giao_dich_thu; i++){
        int res = fprintf(file, "%s, %d, %d, %d, %d, %d, %d, %s, %s\n",
                mang_thu[i].ma_gd, 
                ...
                mang_thu[i].ma_dm,
                mang_thu[i].ma_ns,
                (strlen(mang_thu[i].ghi_chu) > 0) ? mang_thu[i].ghi_chu : "None"); 
        if (res < 0) printf("Loi ghi dong thu %d\n", i);
    }
    ...
    for (int i = 0; i < so_luong_giao_dich_chi; i++){
        int res = fprintf(file, "%s, %d, %d, %d, %d, %d, %d, %s, %s\n",
                ...
                mang_chi[i].ma_dm,
                mang_chi[i].ma_ns,
                (strlen(mang_chi[i].ghi_chu) > 0) ? mang_chi[i].ghi_chu : "None");
        if (res < 0) printf("Loi ghi dong chi %d\n", i);
    }     
            
    fflush(file); // Ép buộc ghi dữ liệu từ bộ đệm vào ổ cứng ngay lập tức
    fclose(file);
    printf("=> Da ket thuc ghi file. Kiem tra file giao_dich.txt ngay!\n");
}
